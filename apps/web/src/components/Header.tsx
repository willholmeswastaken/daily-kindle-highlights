...
const Header = () => {
    ...
    const onSignOut = () => signOut({ callbackUrl: window.location.origin });
    const onSignIn = () => signIn(undefined, { callbackUrl: `${window.location.origin}/books` });
                    ...
                    <div className="px-5 pt-5 pb-6">
                        ...
                        <div className="mt-6">
                            <nav className="grid gap-y-8">
                                {
                                    isLoggedIn && menuItems.map(x => (
                                        <Link href={x.url} className="-m-3 flex items-center rounded-md p-3 hover:bg-gray-50">
                                            {x.icon}
